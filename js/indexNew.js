...
if (myReportId != null && myuserId != null) {
	// alert('wechat');
	getReport();
}
...
setupWebViewJavascriptBridge(function(bridge) {
	...
	// 注册JS方法供OC调用
	bridge.registerHandler('gotoReloadReport', function(data, responseCallback) {
		//alert('哈哈哈哈');
		...
		myReportId = obj.reportId;
		myuserId = obj.userId;
		myclientType = obj.clientType;
	...
	//为按钮注册方法
	$(document).on("click","#goShare",function(){
		//alert('hahahha');
		bridge.callHandler('goToShare', {'reportId':myReportId}, function responseCallback(responseData) {
...
function getReport() {
	...
	$('.guang').css({"transform":"rotate(0deg)","transition":"transform 0s"});
	$.ajax({
	url : dataUrl + "/api/app/report/analysisReport",
	type : "POST",
	dataType : 'json',
	data : {
	    reportId : myReportId,
	    userId : myuserId,
	    clientType: myclientType/*myclientType*/
	},
	success: function(data){
		if(data.code == 200){
			$.ajax({
			url : dataUrl + "/api/app/report/indexAll",
			type : "POST",
			dataType : 'json',
			data : {
			    reportId : myReportId,
			...
			success : function(indexData) {
				if(indexData.code == 201){
					...
					window.location.href="payfor.html?reportId=" + myReportId + '&userId=' + myuserId + "&sameUser=" + sameUser + "&edition="+edition;
				}else if(indexData.code == 200){
					var userId = indexData.data.userId;
					//告诉App报告Id为不收费的
					setupWebViewJavascriptBridge(function(bridge) {
						// JS调用OC注册的方法
						bridge.callHandler('freeReport', {'reportId':myReportId}, function responseCallback(responseData) {})
					});
					...
					if(myApp==null){
						...
						myApp.totalScore = indexData.data.indexPage.totalScore //全部得分
				   		myApp.inspectDate = indexData.data.indexPage.inspectDate // 检测日期
				    	myApp.ranking = indexData.data.indexPage.ranking //排名
				    	myApp.age = indexData.data.indexPage.age//生理年龄
				  		myApp.reportStr = indexData.data.indexPage.reportStr //生理年龄字句
				  		myApp.firstStr = indexData.data.indexPage.firstStr //各个系统生理年龄
				  		myApp.sites = indexData.data.firstPages //各个系统
				  		myApp.status = indexData.data.otherPages //身体状况
					};
					...
					//总分动画效果
					setTimeout(function(){
						$('.guang').css({"transform":"rotate("+1.8*indexData.data.indexPage.totalScore+"deg)","transition":"transform 1s"});
					},100)
					...
					//跳转历史报告
					$('#checkHistory').attr("href","reportHistory.html?&userId=" + userId + "&userId=" + myuserId + '&reportId=' + myReportId);
					//跳转用户设置
...
/*$.ajax({
	...
	error : function(obj,msg){console.log(obj+msg + ":查用户出错");}
			...
			//window.location.href="userInfor.html?reportId=" + myReportId+"&userId=" + data.data.userId + "&userId=" + myuserId + "&edition="+edition;
		}else if(data.code == 403){
			window.location.href="supAge.html?reportId=" + myReportId+"&userId=" + myuserId + "&edition="+edition;
